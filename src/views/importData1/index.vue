...
<script>
  import {fetchList, getTamplate, do_import, doDeleteByIds, exportData,doDeleteHis} from '@/api/importData1'
  ...
  export default {
    ...
    methods: {
      handleCheckedCitiesChange() {
        ...
        Cookies.remove('column2show_import2');
        ...
        Cookies.set('column2show_import2', JSON.stringify(this.column2showData), {expires: 7, path: '/'});
      },
      getColumn() {//回显
        if (Cookies.get('column2show_import2') !== null && Cookies.get('column2show_import2') !== undefined) {
          this.column2showData = JSON.parse(Cookies.get('column2show_import2'));
        } else {
...
// 从后生成字符，向前推进
      ...
      handleUpdate(index, row) {
        this.$router.push({path: '/importData1/updateImportData1', query: {id: row.id}})
      },
      addImportData() {
        this.$router.push({path: '/importData1/createImportData1'});
      },
